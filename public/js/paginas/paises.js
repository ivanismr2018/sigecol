...
function showModalNuevo(){
    ...
    document.getElementById("titulo-paises").innerHTML = "Agregar País";
...
function nuevo() {
    ...
    $.ajax({
        method: "POST",
        data: datosact,
        url: baseurl+"nomencladores/pais/agregar",
...
function cargarDatos(id){
    ...
    document.getElementById("titulo-paises").innerHTML = "Actualizar País";
    ...
    $.ajax({
        "type":"POST",
        "data":{'id':id},
        "url": baseurl+'nomencladores/pais/cargarDatos',
        success:function(value){
          var response = JSON.parse(value);
          $.each(response, function (key, value) {
            $('#id').val(id);
            $('#nombre').val(response['nombre']);
            $('#iso').val(response['iso']);
          });
        }
    });
...
function actualizar() {
    ...
    $.ajax({
        method: "POST",
        data: datosact,
        url: baseurl+"nomencladores/pais/actualizar",
        success:function(data){
          if (data == 1) {
...
function eliminar(id){
    Swal.fire({
        ...
        $.ajax({
            type:"POST",
            data: {'id': id},
            url:baseurl+"nomencladores/pais/eliminar",
            success:function(r){
                if (r==1){
...
function mostrarDatos() {
    ...
    "ajax": {
            "url": baseurl+"nomencladores/pais/listar",
            "type": "POST",
            dataSrc: ''
            },
    "columns": [
        { data: 'iso',
            render: function ( data, type, row ) {
                return "<img src='"+baseurl+"paises/"+data.toLowerCase()+".png' class='img-fluid' alt='' width='50px'>"
            }
        },
        { data: 'nombre' },
        { data: 'iso' },
