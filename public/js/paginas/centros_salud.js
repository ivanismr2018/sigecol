...
function showModalNuevo(){
    ...
    document.getElementById("titulo-centros_salud").innerHTML = "Agregar Centro de Salud";
...
function nuevo() {
    ...
    $.ajax({
        method: "POST",
        data: datosact,
        url: baseurl+"nomencladores/centros_salud/agregar",
        success:function(data){
          if (data == 1) {
                SwalCorto('Información Guardada','success');
                setTimeout(refrescar('tabla'), 1500);
                document.getElementById("frmNuevo").reset();
                $('#centros_saludModal').modal('hide');
          } else {
...
function cargarDatos(id){
    ...
    document.getElementById("titulo-centros_salud").innerHTML = "Actualizar Centro de Salud";
    ...
    $.ajax({
        "type":"POST",
        "data":{'id':id},
        "url": baseurl+'nomencladores/centros_salud/cargarDatos',
        success:function(value){
          var response = JSON.parse(value);
          $.each(response, function (key, value) {
            $('#id').val(id);
            $('#nombre').val(decodeHtml(response['nombre']));
...
function actualizar() {
    ...
    $.ajax({
        method: "POST",
        data: datosact,
        url: baseurl+"nomencladores/centros_salud/actualizar",
        success:function(data){
          if (data == 1) {
...
function eliminar(id){
    Swal.fire({
        ...
        $.ajax({
            type:"POST",
            data: {'id': id},
            url:baseurl+"nomencladores/centros_salud/eliminar",
            success:function(r){
                if (r==1){
...
function mostrarDatos() {
    ...
    "ajax": {
            "url": baseurl+"nomencladores/centros_salud/listar",
            "type": "POST",
            dataSrc: ''
            },
    "columns": [
